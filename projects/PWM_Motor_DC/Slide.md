...
| **1-10 kHz** | Smooth, quiet operation | ✓ Most motors |
| **> 20 kHz** | Very quiet, higher switching loss | Ultrasonic apps |
...
---------          ------------           --------
OC1A (PB5) ────→   ENA (Enable A)
OC1B (PB6) ────→   ENB (Enable B)
PD0        ────→   IN1                    Motor A
PD1        ────→   IN2                      │
PD2        ────→   IN3                    Motor B
PD3        ────→   IN4                      │
...
void timer1_pwm_init(void) {
    ...
    // Prescaler = 8
    // PWM frequency = 16MHz / (8 * 1024) ≈ 1.95 kHz
    TCCR1B |= (1 << CS11);
...
void motor_ramp_speed(uint8_t start, uint8_t end, uint16_t duration_ms) {
    ...
    for (uint16_t i = 0; i <= steps; i++) {
        uint8_t speed = start + (uint8_t)(increment * i);
...
|---------|----------------|----------|
| **Motor doesn't spin** | No power to driver | Check power connections, LED indicators |
| | Wrong wiring | Verify IN1/IN2 connections |
| | Insufficient PWM | Increase duty cycle, check OCR value |
| **Jittery motion** | Low PWM frequency | Increase frequency (1-10 kHz) |
| | Mechanical binding | Check for obstructions |
| | Unstable power | Add capacitors (100µF, 0.1µF) |
| **Wrong direction** | Swapped IN1/IN2 | Swap control pins or motor wires |
| **Overheating** | Overcurrent | Check motor ratings, add heatsink |
| | Continuous stall | Add stall detection |
| **Erratic behavior** | EMI from motor | Add decoupling caps, flyback diodes |
| | Shared ground issues | Use common ground for MCU and driver |
...
✓ **Power Supply**
- Use separate power for motors and MCU
- Add bulk capacitors (100-1000µF) near driver
- Include ceramic bypass caps (0.1µF) on VCC pins
...
uint8_t find_dead_zone(void) {
    for (uint8_t pwm = 0; pwm < 100; pwm++) {
        ...
        if (encoder_pulses > 0) {
            return pwm;  // Motor started moving
        }
    }
    return 0;
}
