/*
 * PWM Motor Control - DC Motor Speed Control
 * ATmega128 Educational Framework
 *
 * LEARNING OBJECTIVES:
 * - Master PWM generation using Timer1 (16-bit)
 * - Control DC motor speed through duty cycle variation
 * - Understand H-Bridge motor driver circuits
 * - Practice motor direction and braking control
 *
 * HARDWARE SETUP:
 * - DC Motor connected via L298N H-Bridge
 * - PWM output on PB5 (OC1A) for speed
 * - Direction control on PB6, PB7
 * - UART for control interface
 * - LEDs on PORTC for status
 *
 * PWM CONCEPTS:
 * - Duty Cycle: Percentage of time signal is HIGH (0-100%)
 * - Frequency: Rate of PWM cycles (typically 1-20kHz for motors)
