 ...
 *
 * LEARNING OBJECTIVES:
 * - Understand different sleep modes
 * - Practice power consumption reduction
 * - Implement low-power applications
 * - Use wake-up sources effectively
 *
 * SLEEP MODES (ATmega128):
 * - Idle: CPU stopped, peripherals running
 * - ADC Noise Reduction: CPU + I/O stopped, ADC running
 * - Power-down: Only asynchronous peripherals active
 * - Power-save: Timer2 + watchdog active (RTC capability)
 ...
 * - Active: ~15 mA @ 8 MHz
 * - Idle: ~5 mA
 * - Power-save: ~5-10 µA (with Timer2)
 * - Power-down: ~1-2 µA
 *
 * WAKE-UP SOURCES:
