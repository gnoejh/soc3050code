 ...
 *
 * LEARNING OBJECTIVES:
 * - Interface with external SPI EEPROM (25LC256 - 32KB)
 * - Implement SPI command sequences
 * - Practice page write and sequential read operations
 * - Master memory addressing and status register handling
 *
 * HARDWARE SETUP:
 * - 25LC256 SPI EEPROM (32KB, 64-byte pages)
 * - PB0 (SS/CS) → Pin 1 (CS) of 25LC256
 * - PB1 (SCK)   → Pin 6 (SCK) of 25LC256
 * - PB2 (MOSI)  → Pin 5 (SI) of 25LC256
 * - PB3 (MISO)  → Pin 2 (SO) of 25LC256
 * - Pin 3 (WP) → VCC (write protect disabled)
 * - Pin 7 (HOLD) → VCC (hold disabled)
 * - Pin 4 (VSS) → GND
 * - Pin 8 (VCC) → +5V
 *
 * 25LC256 COMMANDS:
