 ...
 *               Master mode: Controls ACK/NACK for received data
 *               1 = Send ACK after receiving byte (more data expected)
 *               0 = Send NACK after receiving byte (last byte)
 *               Slave mode: Enables slave to respond to own address
 *
 * TWSTA (bit 5): TWI START Condition
 *                Write 1 to send START (or repeated START) condition
 *                Cleared automatically by hardware after START sent
 *                START: SDA falls while SCL is high
 *                Usage: TWCR = (1<<TWINT)|(1<<TWSTA)|(1<<TWEN);
 *
 * TWSTO (bit 4): TWI STOP Condition
 *                Write 1 to send STOP condition
 *                Cleared automatically by hardware after STOP sent
 *                STOP: SDA rises while SCL is high
 *                Usage: TWCR = (1<<TWINT)|(1<<TWSTO)|(1<<TWEN);
 *
 * TWWC (bit 3): TWI Write Collision Flag
 *               Set if TWDR written while TWINT=0 (illegal operation)
 *               Cleared by writing to TWDR when TWINT=1
 *
 * TWEN (bit 2): TWI Enable
 *               1 = Enable TWI interface (activates SDA/SCL pins)
 *               0 = Disable TWI interface (releases pins to PORT control)
 *               Must be set for all TWI operations
 *
 * TWIE (bit 0): TWI Interrupt Enable
 *               1 = Enable TWI interrupt when TWINT flag is set
 *               0 = Polling mode (check TWINT manually)
 *
 * TYPICAL TWCR COMMAND SEQUENCES:
 *
 *   Send START:
 ...
 *     while(!(TWCR & (1<<TWINT)));
 *
 *   Receive Byte with ACK (more data coming):
 *     TWCR = (1<<TWINT) | (1<<TWEA) | (1<<TWEN);
 *     while(!(TWCR & (1<<TWINT)));
 *     data = TWDR;
 *
 *   Receive Byte with NACK (last byte):
 *     TWCR = (1<<TWINT) | (1<<TWEN);
 *     while(!(TWCR & (1<<TWINT)));
 *     data = TWDR;
 *
 ...
 *
 * TWS7:3 (bits 7-3): TWI Status Code - OPERATION RESULT INDICATOR
 *                    Read after TWINT=1 to determine operation result
 *                    Mask with 0xF8 to ignore prescaler bits
 *                    Usage: status = TWSR & 0xF8;
 *
 *                    MASTER TRANSMITTER STATUS CODES:
 *                    0x08 = START condition transmitted
 *                    0x10 = Repeated START condition transmitted
 *                    0x18 = SLA+W transmitted, ACK received (device ready)
 *                    0x20 = SLA+W transmitted, NACK received (device busy/missing)
 *                    0x28 = Data byte transmitted, ACK received
 *                    0x30 = Data byte transmitted, NACK received
 *
 *                    MASTER RECEIVER STATUS CODES:
 *                    0x38 = Arbitration lost (multi-master conflict)
 ...
 *
 * TWPS1:0 (bits 1-0): TWI Prescaler Bits - SCL FREQUENCY CONTROL
 *                     00 = Prescaler = 1
 *                     01 = Prescaler = 4
 *                     10 = Prescaler = 16
 *                     11 = Prescaler = 64
 *                     Usually set to 00 (prescaler = 1) for simplicity
 *
 * REGISTER 3: TWDR (TWI Data Register) - DATA TRANSFER REGISTER
 *
 * TWDR contains:
 * - When transmitting: Byte to be transmitted on I2C bus
 * - When receiving: Last byte received from I2C bus
 * - During addressing: Device address + R/W bit
 *
 * Device Address Format:
 *   7-bit address: TWDR = (device_addr << 1) | R/W_bit
 *   R/W_bit: 0 = Write (Master Transmitter)
 *            1 = Read  (Master Receiver)
 *   Example: EEPROM at 0x50, write: TWDR = (0x50<<1)|0 = 0xA0
 *            EEPROM at 0x50, read:  TWDR = (0x50<<1)|1 = 0xA1
 *
 * CRITICAL: Only write to TWDR when TWINT=1 to avoid write collision (TWWC)
 *
 * REGISTER 4: TWBR (TWI Bit Rate Register) - SCL FREQUENCY SETTING
 *
 * SCL_frequency = F_CPU / (16 + 2 * TWBR * Prescaler)
 *
 * Common Configurations @ 16MHz:
 *
 *   100kHz (Standard Mode):
 *     TWBR = 72, Prescaler = 1
 *     SCL = 16MHz / (16 + 2*72*1) = 16MHz / 160 = 100kHz
 *
 *   400kHz (Fast Mode):
 *     TWBR = 12, Prescaler = 1
 *     SCL = 16MHz / (16 + 2*12*1) = 16MHz / 40 = 400kHz
 *
 *   50kHz (Slow/Compatible Mode):
 *     TWBR = 152, Prescaler = 1
 *     SCL = 16MHz / (16 + 2*152*1) = 16MHz / 320 = 50kHz
 *
 * NOTE: For 7.3728MHz system (common with UART):
 *   100kHz: TWBR = 32, Prescaler = 1
 *   SCL = 7.3728MHz / (16 + 2*32*1) = 92.16kHz (close to 100kHz)
 *
 * REGISTER 5: TWAR (TWI Address Register) - SLAVE MODE ADDRESSING
 *
 *    Bit:   7      6      5      4      3      2      1      0
 *    Name:  TWA6   TWA5   TWA4   TWA3   TWA2   TWA1   TWA0  TWGCE
 *
 * TWA6:0 (bits 7-1): TWI Slave Address (7-bit)
 *                    Device's own address when operating as slave
 *                    Not used in master-only applications
 *
 * TWGCE (bit 0): TWI General Call Recognition Enable
 *                1 = Respond to general call address (0x00)
 *                0 = Ignore general call
 *
 * TYPICAL INITIALIZATION FOR MASTER MODE @ 16MHz, 100kHz:
 *
 *   void twi_init(void) {
 *       TWBR = 72;              // Bit rate for 100kHz
 *       TWSR = 0x00;            // Prescaler = 1
 *       TWCR = (1<<TWEN);       // Enable TWI
 *       // TWAR not needed for master-only mode
 *   }
 *
 * I2C PROTOCOL TIMING @ 100kHz:
 * - SCL period: 10 µs (100kHz)
 * - SCL high time: 4-5 µs
 * - SCL low time: 4-5 µs
 * - START/STOP setup time: 4.7 µs
 * - Data hold time: 0 µs (internally handled)
