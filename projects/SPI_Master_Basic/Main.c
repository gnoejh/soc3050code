 ...
 *
 * LEARNING OBJECTIVES:
 * - Understand SPI protocol fundamentals (MOSI, MISO, SCK, SS)
 * - Master SPI register configuration (SPCR, SPSR)
 * - Implement master mode data transmission
 * - Practice chip select (SS) management
 *
 * HARDWARE SETUP:
 * - SPI Master: ATmega128
 * - PB0 (SS/CS)   - Chip Select (active low)
 * - PB1 (SCK)     - Serial Clock (master generates)
 ...
 * - Clock Polarity (CPOL): Clock idle state (0=low, 1=high)
 * - Clock Phase (CPHA): Data sampling edge (0=leading, 1=trailing)
 * - MSB First: Most significant bit transmitted first
 * - Speeds: F_CPU/2, /4, /8, /16, /32, /64, /128
 */
