 ...
 *
 * LEARNING OBJECTIVES:
 * - Understand watchdog timer (WDT) operation
 * - Implement system reset mechanisms
 * - Practice fail-safe programming techniques
 * - Handle system crashes and hangs
 *
 * WATCHDOG TIMER OVERVIEW:
 * - Independent oscillator (~1MHz RC)
 * - Timeout periods: 16ms to 2048ms
 * - Generates system reset if not cleared
 * - Useful for detecting software crashes
 *
 * WDTCR Register (Watchdog Control Register):
 * - Bit 4 (WDCE): Watchdog Change Enable
 * - Bit 3 (WDE): Watchdog Enable
 * - Bits 0-2 (WDP): Watchdog Prescaler
 *
 * TIMEOUT PERIODS:
