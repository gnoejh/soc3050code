...
if ($ProjectDir -like "*\projects\*") {
    ...
    elseif ($name -eq "Graphics_Display") {
        ...
        # IMPORTANT: -mshort-calls forces 16-bit addressing (LPM) instead of 24-bit (ELPM)
        # This avoids ELPM bug in SimulIDE 1.1.0-SR1
        $SimulIDENewFlag = "-DSIMULIDE_NEW_VERSION"
        $NoELPMFlag = "-mshort-calls"
        & $avrGccExe $CommonFlags.Split(' ') $McStudioFlags.Split(' ') $ExactMcStudioFlags.Split(' ') $SimulIDENewFlag $NoELPMFlag $SourceFile `
            ../../shared_libs/_glcd.c `
            ../../shared_libs/_port.c `
            ../../shared_libs/_init.c `
            -lm -o "$BaseName.elf"
    }
    elseif ($name -eq "ADC_Basic") {
        Write-Host "Building ADC_Basic with GLCD library (no ELPM for SimulIDE)..." -ForegroundColor Magenta
        # ADC_Basic uses GLCD library with ks0108_printf (needs -mshort-calls like Graphics_Display)
        # IMPORTANT: -mshort-calls avoids ELPM bug in SimulIDE 1.1.0-SR1
        $SimulIDENewFlag = "-DSIMULIDE_NEW_VERSION"
        $NoELPMFlag = "-mshort-calls"
        & $avrGccExe $CommonFlags.Split(' ') $McStudioFlags.Split(' ') $ExactMcStudioFlags.Split(' ') $SimulIDENewFlag $NoELPMFlag $SourceFile `
            ../../shared_libs/_glcd.c `
