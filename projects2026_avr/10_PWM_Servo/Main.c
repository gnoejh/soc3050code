...
void demo1_basic_positioning(void)
{
    ...
    puts_USART1("  a[angle]: Set Servo A (e.g., 'a90' for 90 deg)\r\n");
    puts_USART1("  b[angle]: Set Servo B (e.g., 'b180' for 180 deg)\r\n");
    puts_USART1("  0-9: Quick angles (0=0 deg, 5=90 deg, 9=180 deg)\r\n");
            ...
            if (c == '\r' || c == '\n')
            {
                ...
                if (buf_index > 0)
                {
                    ...
                    else if (input_buffer[0] == 'b' || input_buffer[0] == 'B')
                    {
                        ...
                        char msg[50];
                        sprintf(msg, "Servo B -> %d deg\r\n", angle);
                        puts_USART1(msg);
                    }
                    else if (input_buffer[0] >= '0' && input_buffer[0] <= '9')
                    {
                        uint8_t angle = (input_buffer[0] - '0') * 20; // 0->0 deg, 5->100 deg, 9->180 deg
                        servo_set_angle(SERVO_A, angle);
                        servo_set_angle(SERVO_B, angle);
                        char msg[50];
                        sprintf(msg, "Both servos -> %d deg\r\n", angle);
                        puts_USART1(msg);
                    }
...
void demo2_sweep_test(void)
{
    ...
    while (1)
    {
        // Sweep forward (0 deg to 180 deg)
        puts_USART1("Sweeping forward (0 deg -> 180 deg)...\r\n");
        ...
        // Sweep backward (180 deg to 0 deg)
        puts_USART1("Sweeping backward (180 deg -> 0 deg)...\r\n");
...
void demo3_smooth_movement(void)
{
    ...
    while (1)
    {
        ...
        {
            char buf[50];
            sprintf(buf, "Moving to %d deg (smooth)...\r\n", positions[i]);
            puts_USART1(buf);
...
void demo4_joystick_control(void)
{
    ...
    while (1)
    {
        ...
        // Convert ADC values (0-1023) to servo angles (0-180 deg)
        ...
        {
            char buf[70];
            sprintf(buf, "ADC: X=%4u Y=%4u  |  Servos: A=%3d deg B=%3d deg\r\n",
...
int main(void)
{
    ...
    char buf[80];
    // SERVO_TOP is derived from F_CPU and so has type unsigned long; cast the
    // arguments so they match the %u conversions.
    sprintf(buf, "PWM: %dHz, TOP=%u, Pulse: %u-%u ticks\r\n",
            SERVO_FREQ_HZ, (unsigned)SERVO_TOP,
            (unsigned)SERVO_MIN_PULSE, (unsigned)SERVO_MAX_PULSE);
    puts_USART1(buf);

    // Initialize servos to center position
    servo_set_angle(SERVO_A, 90);
    servo_set_angle(SERVO_B, 90);
    puts_USART1("Servos initialized to 90 deg (neutral)\r\n");
