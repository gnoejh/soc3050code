/*
 * ==============================================================================
 * LCD CHARACTER DISPLAY - DEMO CODE
 * ==============================================================================
 * PROJECT: LCD_Character_Basic
 * See Slide.md for complete theory and technical details
 *
 * DEMOS: LCD initialization, character display, cursor control
 * ==============================================================================
 */
...
void demo4_realtime(void)
{
    ...
    while (1)
    {
        ...
        // Send to UART
        // buf is sized for one 16-column LCD row; the serial line is longer.
        char line[48];
        sprintf(line, "\rTemp:%uC Humid:%u%% Light:%u%%    ", temp, humid, light);
        puts_USART1(line);

        // Progress bar on LEDs
        LED_WRITE((light < 25) ? 0x01 : (light < 50) ? 0x03
                                  : (light < 75)   ? 0x07
                                                   : 0x0F);
