...
/* config.h fixes F_CPU at 16 MHz for the whole 2026 edition and pulls in
 * avr/io.h, util/delay.h and stdint.h. */
#include "config.h"
#include <avr/wdt.h>
...
void phase2_countdown_to_reset(void)
{
    ...
    {
        LED_WRITE(0xAA); // Alternating pattern
        _delay_ms(100);
        LED_WRITE(0x55);
        _delay_ms(100);
...
int main(void)
{
    ...
    LED_WRITE(0x00); // All LEDs OFF initially
