...
void main_serial_polling_single_char(void)
{
     ...
     */
    UART_transmit_char(UART_CHANNEL_1, 'A'); // Send initial character
     ...
     * - Immediately sends it back
     * - Demonstrates bidirectional communication
     */
    while (1)
    {
        /* Modern library approach for reliable reception */
        unsigned char received_char = UART_receive_char(UART_CHANNEL_1);

        /* Echo the character back using library function */
        UART_transmit_char(UART_CHANNEL_1, received_char);

        /*
         * EDUCATIONAL NOTE: Error handling
         * In production code, add error checking:
         * if (UART_get_error_status(UART_CHANNEL_1) != UART_NO_ERROR) {
