...
# 7. AVR-GCC
$AVRGCC = "C:\Program Files (x86)\Atmel\Studio\7.0\toolchain\avr8\avr8-gnu-toolchain\bin\avr-gcc.exe"
Test-Component -Name "AVR-GCC Compiler" -Test {
    Test-Path $AVRGCC
} -SuccessMessage "Found: $AVRGCC" `
  -FailMessage "AVR-GCC not found (required for building)" `
  -FixCommand "Install Microchip Studio or standalone AVR-GCC"
