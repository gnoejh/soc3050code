/*
 * main.c - Phase 0 spike, STM32L031K6
 *
 * Tests whether the C0 lesson code ports to the L0 essentially unchanged
...
static void uart2_init(uint32_t pclk, uint32_t baud)
{
    ...
    RCC->APB1ENR |= RCC_APB1ENR_USART2EN;   /* C0 calls this APBENR1 */
    ...
    GPIOA->AFR[0] |=  ((0x4u << (USART_TX * 4)) | (0x4u << (USART_RX * 4)));  /* AF4 on L0 */
