...
/**
 * @brief Invert all pixels on the screen (black<->white)
 *
 * Reads each byte from the display, inverts it, and writes it back.
 * Useful for creating visual effects or attention-getting displays.
 */
void ks0108_invert_screen(void)
{
    // Invert all pixels by reading, inverting, and writing back
    for (uint8_t page = 0; page < KS0108_PAGES; page++)
    {
        for (uint8_t col = 0; col < KS0108_WIDTH; col++)
        {
            // Determine which controller to use
            uint8_t controller = (col < 64) ? KS0108_LEFT_CONTROLLER : KS0108_RIGHT_CONTROLLER;
            uint8_t local_col = (col < 64) ? col : (col - 64);

            // Set position
            ks0108_set_page(page, controller);
            ks0108_set_column(local_col, controller);

            // Read current byte (dummy read first, then actual read)
            ks0108_read(controller);
            uint8_t data = ks0108_read(controller);
 ...
 * FIXED: Force 16-bit addressing to avoid ELPM instructions
 *
...
void lcd_string_P(byte row, byte col, const char *progmem_str)
{
    ...
    // Use inline assembly with LPM (not ELPM) to read from program memory
    // Force 16-bit address by casting to uint16_t
    uint16_t addr = (uint16_t)progmem_str;

    for (byte i = 0; i < 21; i++)
    {
        uint8_t c;
        __asm__ __volatile__(
            "movw r30, %1\n\t" // Load Z register with address
            "lpm %0, Z\n\t"    // Load byte using LPM (not ELPM)
            : "=r"(c)          // Output
            : "r"(addr)        // Input: 16-bit address
            : "r30", "r31"     // Clobbers Z register
        );
        buffer[i] = c;
        if (c == '\0')
            break;
