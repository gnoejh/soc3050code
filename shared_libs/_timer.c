...
uint16_t Timer_get_compare_value(timer_id_t timer, timer_compare_register_t compare_reg)
{
    switch(timer) {
        ...
        case TIMER_1:
            switch(compare_reg) {
                case TIMER_COMPARE_A: return OCR1A;
                case TIMER_COMPARE_B: return OCR1B;
                case TIMER_COMPARE_C: return OCR1C;
                default: return 0;
            }
        case TIMER_2:
            return OCR2;
        case TIMER_3:
            switch(compare_reg) {
                case TIMER_COMPARE_A: return OCR3A;
                case TIMER_COMPARE_B: return OCR3B;
                case TIMER_COMPARE_C: return OCR3C;
                default: return 0;
            }
        default:
            return 0;
...
void Timer_enable_compare_interrupt(timer_id_t timer, timer_compare_register_t compare_reg)
{
    switch(timer) {
        case TIMER_0:
            ...
            break;
        case TIMER_1:
            if(compare_reg == TIMER_COMPARE_A) TIMSK |= (1<<OCIE1A);
            else if(compare_reg == TIMER_COMPARE_B) TIMSK |= (1<<OCIE1B);
            break;
        case TIMER_2:
            TIMSK |= (1<<OCIE2);
            break;
        case TIMER_3:
            if(compare_reg == TIMER_COMPARE_A) ETIMSK |= (1<<OCIE3A);
            else if(compare_reg == TIMER_COMPARE_B) ETIMSK |= (1<<OCIE3B);
            break;
    }
}
...
uint16_t Timer_calculate_prescaler(timer_id_t timer, uint32_t desired_freq_hz)
{
    ...
    for(uint8_t i = 0; i < 5; i++) {
        ...
        if(max_freq < desired_freq_hz) continue;
...
uint16_t Timer_calculate_compare_value(timer_id_t timer, uint16_t prescaler, uint32_t period_ms)
{
    ...
    if(prescaler > 5) return 0;
    
    uint32_t timer_freq = F_CPU / prescaler_values[prescaler];
    uint32_t ticks_needed = (timer_freq * period_ms) / 1000;
    
    if(timer == TIMER_0 || timer == TIMER_2) {
        if(ticks_needed > 255) return 0;  // 8-bit overflow
    } else {
        if(ticks_needed > 65535) return 0; // 16-bit overflow
    }
...
{
    uint16_t prescaler_values[] = {0, 1, 8, 64, 256, 1024};
    if(prescaler > 5 || compare_value == 0) return 0;
...
{
    if(!scheduler_initialized) return 0xFF;
